...
select r.date, replace(substring_index(l.tag_url,'//',-1),'www.','') tag_url,r.tagid, r.impressions tag, c.stat1, r.served
from kmn_tag_report r
inner join kmn_layouts l on r.tagid=l.layoutid
inner join 
(select
	date, placement_id, sum(impressions) stat1
    from kmn_chain_report r
    ...
    and date between '2016-10-20' and '2016-11-01'
    group by date, placement_id) c on c.placement_id=r.tagid and c.date=r.date
