select l.name,
  ...
  network,
  s.placement_id,
  coalesce(s.filtered,0) filtered0, 
  least(pc_res,26) l_pc_res,
  least(pc_wb,2) l_pc_wb, 
  length(i.uid)>2 as has_cookie,
  count(1) impressions,
  sum(s.cpm) - sum(case when is_winning then received_ssp_bid else 0 end) other_networks_cpm,
  sum(case when is_winning then received_ssp_bid else 0 end) this_network_cpm
from sent_bids s
...
where s.placement_id in ('04fe83cd7b0a314d8e08433d2a5a6b60',
...
and network='sovrn'
and (s.filter_reason = 'npc' or length(s.filter_reason)=0)
and s.timestamp >='2017-06-02'
group by l.name, l.tag_url, s.placement_id, filtered0, l_pc_res, l_pc_wb, has_cookie, network
