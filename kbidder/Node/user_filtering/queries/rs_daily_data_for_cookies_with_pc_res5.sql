select l.name,
  ...
  u.uid,
  s."timestamp",
  coalesce(s.filtered,0) filtered, 
  pc_res,
  pc_wb, 
  wb_at_5_res,
  case when is_winning then s.cpm else 0 end this_network_cpm 
from sent_bids s
...
inner join tmp_uid5 u on u.placement_id = s.placement_id and u.uid = i.uid
where network='sovrn'
and pc_res between 5 and 20
and s.filtered = 0
and s.timestamp between '2017-06-04' and '2017-06-06'
and right(u.uid,2) in ('0a', '1a' ,'2a','3a', '4a', '5a','6a','7a','8a')
...
-- order by placement_id, uid, "timestamp"
