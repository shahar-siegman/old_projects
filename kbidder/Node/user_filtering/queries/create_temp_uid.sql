...
create table tmp_uid4 as
select placement_id, uid 
from (
  select s.placement_id, uid, sum(pc_wb) wb_at_5_res
  from sent_bids s
  ...
  and network='sovrn'
  and length(s.filter_reason)=0
  and s.filtered =0
  and length(i.uid) > 2
  and pc_res=5
  and s.timestamp between '2017-06-04' and '2017-06-06'
  group by s.placement_id, uid
)
  where wb_at_5_res =0
;
select min(length(uid)), max(length(uid)) from tmp_uid;

select count(1) from tmp_uid4
