 select 
      ...
      length(i.uid)> 0 has_cookie,
      count(1) impressions,
      sum(case when s.received_ssp_bid>0 then 1 else 0 end) with_bid,
      sum(s.received_ssp_bid) bid_value,
      sum(case when left(s.kb_sold_tag,1)='e' and network='openx'
          ...
          then s.received_ssp_bid else 0 end) win_value
  from sent_bids s
  ...
  where s.timestamp between '2017-06-09' and '2017-06-12 23:59:59'
