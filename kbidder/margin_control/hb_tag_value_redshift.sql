...
select  placement_id ,
		date_trunc('day',"timestamp")::date date_ ,
		lower(served_tag_network) "network"
		round(sum(received_ssp_bid)/1000,3) hb_tag_value,
FROM sent_bids s
WHERE "timestamp" BETWEEN dateadd('d', -6, CURRENT_DATE) AND dateadd('d', -1, CURRENT_DATE)
	AND code=kb_sold_tag
	and kb_sold_tag != ''
GROUP BY placement_id,
         date_,
